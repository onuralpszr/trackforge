...
def test_ocsort_constructor():
    t = trackforge.OCSORT(max_age=30, min_hits=1, iou_threshold=0.3, delta_t=3, inertia=0.2)
...
def test_ocsort_confirmed_after_min_hits():
    t = trackforge.OCSORT(max_age=30, min_hits=3, iou_threshold=0.3, delta_t=3, inertia=0.2)
...
def test_deepsort_constructor():
    t = trackforge.DEEPSORT(max_age=70, n_init=3, max_iou_distance=0.7,
...
def test_deepsort_update_with_embedding():
    t = trackforge.DEEPSORT(max_age=70, n_init=1, max_iou_distance=0.7,
                             max_cosine_distance=0.2, nn_budget=100)
    det = [([100.0, 100.0, 50.0, 100.0], 0.9, 0)]
    emb = [[0.1] * 128]
...
def test_bytetrack_update_returns_track():
    t = trackforge.BYTETRACK(track_thresh=0.5, track_buffer=30,
                              match_thresh=0.8, det_thresh=0.6)
    tracks = t.update([([100.0, 100.0, 50.0, 100.0], 0.9, 0)])
    assert len(tracks) == 1
